  ...
  try {
    ...
    if (!isHubSpotConfigured()) {
      return res.status(400).json({ 
        message: "HubSpot API credentials not configured. Please add HUBSPOT_CLIENT_ID and HUBSPOT_CLIENT_SECRET to your environment variables." 
      });
    }
...
export async function hubspotCallback(req: Request, res: Response) {
  try {
    console.log("Received HubSpot OAuth callback");
    ...
    const { 
      access_token, 
      refresh_token, 
      expires_in = 21600 // Default to 6 hours if not provided
    } = tokenResponse.data;
    
    console.log("Successfully obtained HubSpot access token");
    ...
    console.log(`Connected to HubSpot account: ${accountName}`);
    ...
    console.log("Creating new HubSpot platform connection");
    const newPlatform = await storage.createPlatform({
      ...
      tokenExpiry,
      metadata: {
        hubId: accountInfo.data.hub_id,
        portalId: accountInfo.data.portal_id,
        hubDomain: accountInfo.data.hub_domain
      },
      isConnected: true
    });
...
async function refreshHubSpotToken(platform: Platform): Promise<Platform> {
  try {
    ...
    const { access_token, refresh_token, expires_in = 21600 } = tokenResponse.data;
...
async function getValidHubSpotToken(platform: Platform): Promise<string> {
  ...
  if (expiresInMs < 5 * 60 * 1000) {
    ...
    return refreshedPlatform.accessToken as string;
  }
  
  return platform.accessToken as string;
}
...
export async function getHubSpotContacts(req: Request, res: Response) {
  try {
    ...
    // Check if we're using real HubSpot credentials
    if (isHubSpotConfigured() && platform.accessToken && !platform.accessToken.startsWith('dev-')) {
      try {
        ...
        const response = await axios.get('https://api.hubapi.com/crm/v3/objects/contacts', {
          ...
          params: {
            limit: 10,
            properties: 'firstname,lastname,email,phone,company,hs_object_id,createdate,lastmodifieddate'
          }
        });
        ...
        const contacts = response.data.results.map((contact: any) => {
          return {
            ...
            lastActivity: contact.properties.lastmodifieddate || contact.properties.createdate || null
          };
        });
