...
import * as slackService from "./platforms/slack";
import * as emailService from "./platforms/email-service";
import * as hubspotService from "./platforms/hubspot-service";
import * as salesforceService from "./platforms/salesforce-service";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.delete("/api/platforms/:id", isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.get("/api/platforms/slack/status", isAuthenticated, slackService.getSlackStatus);
  app.get("/api/platforms/slack/connect", isAuthenticated, slackService.connectSlack);
  app.get("/api/platforms/slack/channels", isAuthenticated, slackService.getSlackChannels);
  app.get("/api/platforms/slack/members", isAuthenticated, slackService.getSlackMembers);
  app.get("/api/platforms/slack/messages", isAuthenticated, slackService.getSlackMessages);
  app.post("/api/platforms/slack/send", isAuthenticated, slackService.sendSlackMessage);

  // Email integration routes
  app.get("/api/platforms/email/status", isAuthenticated, emailService.getEmailStatus);
  app.post("/api/platforms/email/connect", isAuthenticated, emailService.connectEmail);
  app.get("/api/platforms/email/messages", isAuthenticated, emailService.getEmails);
  app.post("/api/platforms/email/send", isAuthenticated, emailService.sendEmail);

  // HubSpot integration routes
  app.get("/api/platforms/hubspot/status", isAuthenticated, hubspotService.getHubSpotStatus);
  app.get("/api/platforms/hubspot/connect", isAuthenticated, hubspotService.connectHubSpot);
  app.get("/api/platforms/hubspot/contacts", isAuthenticated, hubspotService.getHubSpotContacts);
  app.post("/api/platforms/hubspot/contacts", isAuthenticated, hubspotService.createHubSpotContact);
  app.get("/api/platforms/hubspot/deals", isAuthenticated, hubspotService.getHubSpotDeals);
  app.post("/api/platforms/hubspot/deals", isAuthenticated, hubspotService.createHubSpotDeal);

  // Salesforce integration routes
  app.get("/api/platforms/salesforce/status", isAuthenticated, salesforceService.getSalesforceStatus);
  app.get("/api/platforms/salesforce/connect", isAuthenticated, salesforceService.connectSalesforce);
  app.get("/api/platforms/salesforce/callback", isAuthenticated, salesforceService.salesforceCallback);
  app.get("/api/platforms/salesforce/leads", isAuthenticated, salesforceService.getSalesforceLeads);
  app.post("/api/platforms/salesforce/leads", isAuthenticated, salesforceService.createSalesforceLead);
  app.get("/api/platforms/salesforce/opportunities", isAuthenticated, salesforceService.getSalesforceOpportunities);
  app.post("/api/platforms/salesforce/opportunities", isAuthenticated, salesforceService.createSalesforceOpportunity);
  ...
  app.get("/api/poll", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      
      // Get any pending messages for this user
      const messages = pendingMessages.get(userId) || [];
      
      // Clear the pending messages (they've been delivered)
      pendingMessages.set(userId, []);
